...
#ifdef CONFIG_APP_NETWORK_PROV_SHOW_QR
    // Desabilita exibição do QR code para testar apenas BLE
...
{
    // Garante nome no formato PROV_XXXXXX (6 hex do MAC)
    uint8_t mac_addr[6];
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 1, 0)
...
#endif
    snprintf(service_name, max, "PROV_%02X%02X%02X", mac_addr[3], mac_addr[4], mac_addr[5]);
    return ESP_OK;
}
...
{
    ESP_LOGI(TAG, "[DEBUG] >>> app_network_start: Iniciando provisioning BLE <<<");
    char *pop = get_device_pop(pop_type);
    if ((pop_type != POP_TYPE_NONE) && (pop == NULL)) {
        ESP_LOGE(TAG, "[DEBUG] >>> PoP inválido ou não definido! <<<");
        return ESP_ERR_NO_MEM;
    }
    ...
    ESP_LOGI(TAG, "[DEBUG] >>> Nome do serviço BLE: %s <<<", service_name);
    /* Log UUID padrão do serviço BLE */
    const uint8_t uuid[16] = {0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00};
    ESP_LOGI(TAG, "[DEBUG] >>> UUID do serviço BLE (esperado): 0000ffff-0000-1000-8000-00805f9b34fb <<<");
    ESP_LOG_BUFFER_HEX_LEVEL(TAG, uuid, 16, ESP_LOG_INFO);
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "[ERRO] >>> Falha ao iniciar provisioning BLE: %s (0x%x) <<<", esp_err_to_name(err), err);
    }
#endif
#ifdef CONFIG_ESP_RMAKER_NETWORK_OVER_THREAD
    ...
    if (!provisioned) {
#ifdef CONFIG_APP_NETWORK_PROV_TRANSPORT_BLE
        ESP_LOGI(TAG, "[DEBUG] >>> Provisionamento BLE ativo. Aguarde conexão do app. <<<");
    ...
    ESP_LOGI(TAG, "[DEBUG] >>> app_network_start: Fim do provisioning BLE <<<");
